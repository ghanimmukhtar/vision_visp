...
//    rectifiedImageTopic_ =
//            ros::names::resolve(cameraPrefix + "/image_rect_color");
    rectifiedImageTopic_ =
            ros::names::resolve(cameraPrefix + "/image_color_rect");
//    rectifiedImageTopic_ =
...
TrackerViewer::waitForImage()
{
    ...
    while (!exiting()
           && (!image_.getWidth() || !image_.getHeight()))
    {
        ROS_INFO_THROTTLE(1, "waiting for a rectified image... tracker viewer ... ");
        ROS_WARN_STREAM("the image topic name: " << rectifiedImageTopic_ << " the image width is: " << image_.getWidth()
                        << " the height is: " << image_.getHeight() << " and exiting is: " << exiting());
        ros::spinOnce();
        loop_rate.sleep();
    }
}
...
{
    ROS_INFO_STREAM("Other Callback, trying to process the kinect image, with width = " << image->width << " and height = " << image->height);
    // Copy image.
    try
    {
    ...
    {
        ROS_ERROR_STREAM("dropping frame in other callback: " << e.what());
    }
