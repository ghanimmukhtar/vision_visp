...
//    rectifiedImageTopic_ =
//            ros::names::resolve(cameraPrefix_ + "/image_rect_color");
    rectifiedImageTopic_ =
            ros::names::resolve(cameraPrefix_ + "/image_color_rect");
//    rectifiedImageTopic_ =
...
TrackerClient::spin()
{
    ...
    while (!ok && !exiting())
    {
        try
        {
            ...
            ROS_WARN_STREAM("The start from saved pose is: " << startFromSavedPose_);
        ...
        {
            mutex_.unlock();
            ROS_ERROR_STREAM("failed to initialize, tracker client number 1: "
                             << e.what() << ", retrying...");
        }
        catch(const std::string& str)
        {
            mutex_.unlock();
            ROS_ERROR_STREAM("failed to initialize, tracker client number 2: "
                             << str << ", retrying...");
        }
        catch(...)
        {
            mutex_.unlock();
            ROS_ERROR("failed to initialize, tracker client number 3 retrying...");
        }
    }
...
TrackerClient::waitForImage()
{
    ...
    while (!exiting()
           && (!image_.getWidth() || !image_.getHeight()) && !info_)
    {
        ROS_INFO_THROTTLE(1, "waiting for a rectified image... tracker client ...");
        ros::spinOnce();
        loop_rate.sleep();
    }
}
