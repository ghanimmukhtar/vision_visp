                    ...
                    const sensor_msgs::Image::ConstPtr& src)
{
    ...
    //using sensor_msgs::image_encodings::
    ...
    if(src->encoding == MONO8) {
        memcpy(dst.bitmap, &src->data[0], dst.getHeight () * src->step * sizeof(unsigned char));
            ...
            || src->encoding == BGR8 || src->encoding == BGRA8 || src->encoding == TYPE_32FC1)
    {
        unsigned nc = numChannels(src->encoding);
        unsigned cEnd =
                (src->encoding == RGBA8 || src->encoding == BGRA8 || src->encoding == TYPE_32FC1) ? nc - 1 : nc;
